        ...
        <div class="box">
            ...
            @if($CheckLayoutPermission[0]->create===1)
                <div class="row p-a pull-right" style="margin-top: -70px;">
                    <div class="col-sm-12">
                        <a class="btn btn-fw primary" href="{{route("testimonialCreate")}}">
                            &nbsp; New Testimonial
                        </a>
                    </div>
                </div>
...
<table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                        ...
                        @foreach($Testimonial as $testimonial)
                            <tr>
                                ...
                                <td>
                                    <small>{!! $testimonial->contents   !!}</small>
                                </td>
                                <td>
                                    <small>{!! $testimonial->company_link   !!}</small>
                                </td>
                                 <td>
                                    <small>{!! $testimonial->video_link   !!}</small>
                                </td>
                                 <td>
                                    <small>{!! $testimonial->created_at   !!}</small>
                                </td>
                                <td>
                                    <small>{!! $testimonial->updated_at   !!}</small>
                                </td>

                                <td class="text-center">
                                @if($CheckLayoutPermission[0]->edit===1)
                                    <a class="btn btn-sm success"
                                       href="{{ route("testimonialEdit",["id"=>$testimonial->id]) }}">
                                    ...
                                    <a class="btn btn-sm warning"
                                       href="{{ route("testimonialDestroy",["id"=>$testimonial->id]) }}">
